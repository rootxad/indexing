    ...
    <v-app>
        <v-app-bar app color="primary" dark>
            <v-toolbar-title class="headline pointer mr-3 hidden-sm-and-down">
                <router-link :to="{ path: '/', query: { rootId: $route.query.rootId } }" tag="span">{{
                    title
                }}</router-link>
            </v-toolbar-title>
            <v-toolbar-items>
                <template v-if="drives.length === 1">
                    <v-btn text class="text-none" @click="changeDrive(drives[0].value)">
                    ...
                    <v-menu offset-y v-if="drives.length > 1">
                        <template v-slot:activator="{ on }">
                            <v-btn text v-on="on" class="text-none">
                                <v-icon>mdi-cloud</v-icon>&nbsp;{{ currentDrive.text }}<v-icon>mdi-menu-down</v-icon>
                            </v-btn>
                        </template>
                        <v-list>
                            <v-list-item
            ...
            <v-toolbar-items>
                <v-btn
                    ...
                    target="_blank"
                >
                    <v-icon>mdi-github-circle</v-icon>&nbsp;GitHub Test</v-btn
                >
            </v-toolbar-items>
        </v-app-bar>
                </template>
...
export default {
    ...
    methods: {
        changeDrive(drive) {
            const rootId = drive !== window.props.default_root_id ? drive : undefined;
            const dest = { path: '/', query: { rootId } };
            if (dest.path === this.$route.path && dest.query.rootId === this.$route.query.rootId) {
